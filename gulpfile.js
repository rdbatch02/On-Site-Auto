...
gulp.task('jshint', function() {
    ...
    '!app/scripts/jquery.js'
        ...
        stream: true,
        once: true
    })).pipe($.jshint()).pipe($.jshint.reporter('jshint-stylish')).pipe(
...
gulp.task('styles', function() {
    ...
    'node_modules/bootstrap/dist/css/bootstrap.min.css'
  ]).pipe($.sourcemaps.init()).pipe($.changed('.tmp/styles', {
            extension: '.css'
        })).pipe($.sass({
            precision: 10,
        ...
        .pipe($.if('*.css', $.csso())).pipe(gulp.dest('dist/styles')).pipe(
            $.size({
                title: 'styles'
            }));
});
...
gulp.task('serve', ['styles'], function() {
    del.bind(null, ['app/scripts/bootstrap.js'], {
        dot: true
    });
    gulp.src([
